...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise SystemExit(
            f"execution reconciliation refused for {label}: expected one target, found {count}"
        )
...
def main() -> int:
    ...
    if "C3 adoption execution is not implemented" in source:
        raise SystemExit("execution reconciliation refused: production service is not present")
    source = replace_once(
        source,
    ...
    print("hardened integrated C3 execution pre-effect failures")
    return 0
