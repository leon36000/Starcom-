...
PATH = Path("src/starcom/recollection.py")
MARKER = '    """RED contract seam for the C1-gated Task 5 recollection coordinator."""\n'

PRODUCTION = '''from __future__ import annotations
...
class C2RecollectionService:
    ...
    def _require_clean_published_c1(self, incident_id: str) -> None:
        ...
        if incident.status is not IncidentStatus.RECOVERY_PUBLISHED_RECOLLECT_REQUIRED:
...
def main() -> int:
    ...
    if source.count(MARKER) != 1:
        raise SystemExit("bounded patch refused: RED C2 seam marker missing or duplicated")
    if source.count("C2 recollection gate is not implemented") != 1:
        raise SystemExit("bounded patch refused: expected exactly one unimplemented C2 gate")
    PATH.write_text(PRODUCTION, encoding="utf-8")
    print("replaced exact RED C2 seam with fail-closed recollection gate")
    return 0
