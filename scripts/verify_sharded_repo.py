...
from scripts.manifest_shards import (  # noqa: E402
    discover_manifest_shards,
...
def main() -> int:
    ...
    shard_paths = discover_manifest_shards(ROOT)
    manifest_error: str | None = None
    try:
        expected = load_manifest_shards(shard_paths)
    except ValueError as exc:
        expected = {}
        manifest_error = str(exc)

    style_findings = []
    for relative in [*sorted(expected), *(path.name for path in shard_paths)]:
    ...
    if manifest_error is not None:
        print(json.dumps({"ok": False, "error": manifest_error}, sort_keys=True))
        steps.append(StepResult("manifest", False, 1, ("in-process",)))
    else:
        manifest = verify_manifest_shards(ROOT, shard_paths)
        print(
            json.dumps(
                asdict(manifest)
                | {
